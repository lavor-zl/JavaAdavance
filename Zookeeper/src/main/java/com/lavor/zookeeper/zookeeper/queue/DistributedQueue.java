...
package com.lavor.zookeeper.zookeeper.queue;
...
public class DistributedQueue {
    ...
    private void init(){
        ...
        try {
            ...
            stat = zookeeper.exists(dir,false);
            System.out.println("exists2");
            if (stat==null){
                System.out.println("开始穿件");
    ...
    public boolean offer(byte[] data) throws KeeperException, InterruptedException{
        for(;;){
            try{
                System.out.println("开始创建子节点");
                zookeeper.create(dir+"/"+prefix, data, acl, CreateMode.PERSISTENT_SEQUENTIAL);
                System.out.println("创建子节点成功");
                return true;
            }catch(KeeperException.NoNodeException e){
                System.out.println("创建子节点失败");
                zookeeper.create(dir, new byte[0], acl, CreateMode.PERSISTENT);
                System.out.println("创建父节点失败");
            }
        }
